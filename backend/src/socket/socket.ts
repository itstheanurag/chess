...
export const initializeSocket = (nsp: Namespace, namespaceType: 'game' | 'chat' = 'game') => {
    ...
    if (namespaceType === 'chat') {
      ...
      socket.on('joinGame', (data: JoinGameData) => {
        ...
        try {
          ...
          console.log(`${playerName} joined ${isSpectator ? 'as spectator in' : 'game in'} room: ${room}`);
        } catch (error) {
...
// Export individual handlers for testing if needed
